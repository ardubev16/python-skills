...
description: Use when setting up or auditing linting, formatting, type checking, pre-commit hooks, or dependency-update automation for a Python project. Configures ruff (lint + format, select=ALL), the ty type checker, a pre-commit pipeline, and Renovate, matching the conventions used in ardubev16/carpoolerbot.
---
...
If the project isn't part of the `ardubev16` org (no shared `common`
preset to extend), fall back to `"extends": ["config:recommended"]` plus
explicit enablement of the `pre-commit` and `github-actions` managers:
